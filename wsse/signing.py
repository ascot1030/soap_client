...
from .encryption import create_binary_security_token
...
def sign(envelope, keyfile, certfile):
    ...
    _sign_node(ctx, signature, header.find(ns('http://ebs.health.ontario.ca/', 'EBS')))
    _sign_node(ctx, signature, header.find(ns('http://idp.ebs.health.ontario.ca/', 'IDP')))
