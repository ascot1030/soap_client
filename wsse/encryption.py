...
from .constants import BASE64B, X509TOKEN, DS_NS, ENC_NS, SOAP_NS, WSSE_NS, WSU_NS
...
def decrypt(envelope, keyfile):
    ...
    for ref in ref_list:
        ...
        # ID might have a 'wsu' prefix or it might not have any prefix - try to find either one
        node = doc.xpath(
            "//enc:EncryptedData[@wsu:Id='%s']" % referenced_id,
            namespaces={'enc': ENC_NS,'wsu':WSU_NS},
        )
        if not node:
            node = doc.xpath(
        ...
        enc_data = node[0] #grab only the first element
        # print("data:",etree.tostring(enc_data).decode('utf-8'))
        ...
        # trying to decrypt.
        # ki = etree.Element(ns(DS_NS,'KeyInfo'))
        # ki.append(etree.Element(ns(DS_NS,'RemoveMe')))
        # enc_data.append(ki)
