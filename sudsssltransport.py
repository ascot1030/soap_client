...
# suppress the warning about unverified HTTPS connection
...
class _StrictSSLHTTPTransportAuthenticated(Transport):
    ...
    def __init__(self, **kwargs):
        ...
        # these will be passed to Client()
        self.suds_kwargs = {
        ...
        # these will be passed to Transport()
        argsForTransport = {
            key: value for key, value in list(kwargs.items()) if key not in (
                'username', 'password', 'client_cert', 'plugins', 'wsse', 'soapheaders',
                'verify_ssl', 'proxy', 'rewrite_to_https'
            )
        }
        Transport.__init__(self, **argsForTransport)
